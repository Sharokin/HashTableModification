...
void test(void)
{
    ...
    assert(htable.table[8].key == 790408);
    /*Genom att ška storleken till 20 fŒr vi en ny tabell dŠr de 4 insŠttningarna šver ^^^
     gŒr šver till den nya tabellen. Assertsen under bekrŠftar att datat ligger pŒ rŠtt index*/
    
  
    HashTable newTable = examinationFunc(&htable, 20); //EXAMINATION
    if(newTable.size == htable.size) //EXAMINATION
        printf("Function did not go execute, size possibly the same, not big enough for amount of elements or less then zero\n\n");
        
    assert(newTable.table[14].key == 931014); //EXAMINATION
    assert(newTable.table[11].key == 881011); //EXAMINATION
    assert(newTable.table[4].key == 600704); //EXAMINATION
    assert(newTable.table[8].key == 790408); //EXAMINATION
    printf("Examination func passed the test\n\n");
